...
{% if msg.files %}
    ...
    {% for file in msg.files %}
        <a href="{{ _prefix_local_file(file) }}" class="gradio-file" target="_blank">📁 {{ _basename(file) }}</a>
    {% endfor %}
    </div>
{% endif %}
{% if msg.buttons %}
    ...
    <div class="references">
        {% for ref in msg.references %}
            ...
            {% for source in ref.sources %}
                <li><a href="{{ source.link }}" class="gradio-file" target="_blank">{{ source.text }}</a> 
                {% if source.score %}
...
def render_message(msg_dict, format='html'):
    if format == 'html':
        ...
        msg = replace_extra_newlines(markdownify(render_message(msg_dict, format='html')))
    elif format == 'plain':
